...
export default function DraftCreateForm() {
  const { classes } = useStyles();
  const { postId } = useParams();

  const { data: draft } = useDraftPost(postId);
  ...
  const {
    mutate: patchDraftToPost,
    isLoading,
    isSuccess
  } = usePatchDraftToPost(postId);
  ...
  const [richText, setRichText] = useState(
    localStorage.getItem(`draft-${postId}`)
  );
      ...
      <Formik
        ...
        onSubmit={() => {
          ...
          patchDraftToPost({
            ...
            about: selectValue,
            willNotify,
            createdAt: Date.now() + (hours || 0) * 1000 * 60 * 60,
            lastCommentedAt: Date.now() + (hours || 0) * 1000 * 60 * 60,
            draft: false,
            poll: pollArray,
          ...
          <form onSubmit={formik.handleSubmit}>
            <div className={classes.form}>
              <TextInput
                ...
                id={`draft-${postId}-title`}
                name={`draft-${postId}-title`}
                key={`draft-${postId}-title`}
                value={draftTitle}
              ...
              <RichTextEditor
                placeholder="Content..."
                mt="md"
                id={`draft-${postId}-content`}
                name={`draft-${postId}-content`}
                key={`draft-${postId}-content`}
                value={richText}
